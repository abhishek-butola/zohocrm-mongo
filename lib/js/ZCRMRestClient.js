...
var mysql_module = './mongo/mongo_util'; //cambiado a mongo
...
var model = require('./mongo/oauthtokens');
...
ZCRMRestClient.initialize = function(configJSON) {
  return new Promise(function(resolve, reject) {
    ...
    var iam_url = properties.get('zoho.crm.iamurl')
      ? properties.get('zoho.crm.iamurl')
      : iamurl;

    var config_properties = PropertiesReader(
      'resources/configuration.properties'
    );

    mysql_module = config_properties.get('crm.api.tokenmanagement')
      ? config_properties.get('crm.api.tokenmanagement')
      : mysql_module;

    baseURL = config_properties.get('crm.api.url')
      ? config_properties.get('crm.api.url')
      : baseURL;

    mysql_username = config_properties.get('mysql.username')
      ? config_properties.get('mysql.username')
      : mysql_username;

    mysql_password = config_properties.get('mysql.password')
      ? config_properties.get('mysql.password')
      : mysql_password;

    if (config_properties.get('crm.api.user_identifier')) {
      ZCRMRestClient.setUserIdentifier(
        config_properties.get('crm.api.user_identifier')
      );
    } else {
  ...
  return new Promise(function(resolve, reject) {
    ...
    OAuth.generateTokens(api_url).then(function(response) {
      ...
      if (resultObj.access_token) {
        ...
        var refresh_token;
        // var mongRes = model.findOne({"user_identifier": resultObj.user_identifier}, (err, oauthtoken) => {
        //     if (err) throw new Error(err);
        //     ZCRMRestClient.generateAuthTokenfromRefreshToken(resultObj.user_identifier,oauthtoken.refresh_token).then(function(auth_response){
...
ZCRMRestClient.parseAndConstructObject = function(response) {
  ...
  if (body.access_token) {
    ...
    resultObj.expires_in = body.expires_in * 1000 + current_time;
  }
  return resultObj;
};
